...
uint8_t epd_is_busy() {
	return HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_4) == GPIO_PIN_RESET ? 0 : 1;
}

void epd_write_reg(uint8_t reg) {
	epd_dc_reset();
	epd_cs_reset();
	HAL_SPI_Transmit(&hspi2, &reg, sizeof(reg), HAL_MAX_DELAY);
...
void epd_address_set(uint16_t x_start, uint16_t y_start, uint16_t x_end,
		uint16_t y_end) {
...
void epd_paint_showString(uint16_t x, uint16_t y, uint8_t *chr, uint16_t size1,
		uint16_t color) {
	while (*chr != '\0') {
