...
uint16_t timer_stop(int16_t start_time) {
	uint16_t stop_time = start_time - __HAL_TIM_GET_COUNTER(&htim2);
	return stop_time;
}
 ...
 */
int64_t rand64() {
	int64_t number = (((uint64_t) (rand()) << 32) | rand());
	return number;
}
...
//	(void) dump32; // Shutup warning
	...
	sprintf((char*) TX_buffer, "add32 = %lu \n\r", add32); // add32 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "add64 = %lu \n\r", add64); // add64 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "mult32 = %lu \n\r", mult32); // mult32 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "mult64 = %lu \n\r", mult64); // mult64 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "div32 = %lu \n\r", div32); // div32 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "div64 = %lu \n\r", div64); // div64 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "byte8 = %lu \n\r", byte8); // byte8 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "byte128 = %lu \n\r", byte128); // byte128 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
	sprintf((char*) TX_buffer, "byte1024 = %lu \n\r", byte1024); // byte1024 test
	for (int i = 0; i < sizeof(TX_buffer); i++) {
		HAL_UART_Transmit(&huart2, TX_buffer + i, 1, UART_timeout);
	}
}
