...
int main(void)
{
  ...
  const uint8_t cls[] = "\033[2J\033[H"; // Clear screen and home cursor
  const uint8_t restore_cursor[] = "\033[?25h"; // Make sure cursor not hidden

  HAL_UART_Transmit(&huart2, cls, sizeof(cls), UART_timeout);
  HAL_UART_Transmit(&huart2, restore_cursor, sizeof(restore_cursor), UART_timeout);
  /* USER CODE END 2 */
